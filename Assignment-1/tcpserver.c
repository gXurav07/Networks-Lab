...
int main()
{
	...
	/* The following system call opens a socket. 
	   The first parameter indicates the family of the protocol to be followed. For internet protocols we use AF_INET.
	   For TCP sockets the second parameter is SOCK_STREAM. 
	   The third parameter is set to 0 for user applications (0 implies default protocol i.e. TCP).
	*/
	...
	*/
	...
	serv_addr.sin_port = htons(20000);		// specifies the port number of the server. htons() provides a format
	...
	*/
	while (1) {
		...
		*/
		...
		strcpy(buf,"My roll number is 20CS10029");
